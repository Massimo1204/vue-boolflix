<template>
  <div v-if="joinShows[0] != null && joinShows[1] != null">
    <div v-for="(show, showIndex) in joinShows" :key="showIndex">
      <div>
        <h1 class="ms-3 pt-3 mb-0">
          {{ showIndex == 0 ? "Movies" : "Series" }}
        </h1>
      </div>
      <div class="d-flex justify-content-start flex-wrap">
        <div
          ...
          :key="index + (showIndex == 0 ? 'movie' : 'series') + element.id"
          @mouseover="toShowId = element.id"
          @mouseenter.once="overviewHeight(showIndex, index)"
          ...
          <div v-show="toShowId == element.id" ref="cardBack">
            ...
            <div v-if="joinCasts[showIndex].length != 0">
              <h6 class="cast" ref="cast">
                Cast:
                <span v-for="n in joinCasts[showIndex][index].length" :key="n">
                  {{
...
export default {
  name: "showCard",
  props: ["movies", "tvSeries", "moviesCasts", "seriesCasts"],
  data: function () {
    return {
