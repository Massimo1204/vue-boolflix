    ...
    <div class="d-flex justify-content-start flex-wrap">
      ...
      >
        <div v-show="toShowId == element.id" ref="cardBack">
          ...
          <div>
            <h6 v-if="cast" ref="cast">
              Cast:
              <span v-for="n in 5" :key="n"> {{ cast[n - 1].name }} </span>
            </h6>
          </div>
          <div>
...
export default {
  ...
  methods: {
    overviewHeight(index) {
      this.$refs.overview[index].style.height =
        275 -
        (this.$refs.title[index].clientHeight +
          this.$refs.originalTitle[index].clientHeight +
          this.$refs.rating[index].clientHeight +
          this.$refs.language[index].clientHeight) +
        "px";
    },
    test() {
